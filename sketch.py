...
from PIL import Image   # Python Image Library  사용 위해

root = Tk()
root.title("Find Error")

# 파일 추가
def add_file():
    files = filedialog.askopenfilenames(title = "Select Image File", \
        filetypes = (("PNG file", "*.png"), ("All file","*.*")), \
        initialdir = r"C:\Users\sangj\OneDrive\바탕 화면\공부\Python\SUMMIT\GuiProgramming2") 
...
def merge_image():
    ...
    if img_space == "Narrowly":
        img_space = 30
    elif img_space == "normal":
        img_space = 60
    elif img_space == "widely":
        img_space = 90
    else: # 없음
        img_space = 0
        
    # 포맷
    img_format = cmb_format.get().lower()   # PNG, JPG, BMP를 받아와 소문자로 변경
    ...
    for idx, img in enumerate(images):
        ...
        progress = (idx + 1) / len(images) * 100 # 실제 percent 정도를 계산 
        p_var.set(progress)
        progress_bar.update()
    ...
    msgbox.showinfo("Alarm", "Job's finished!")
...
def start():   
    ...
    if len(txt_dest_path.get()) == 0: 
        msgbox.showwarning("WARNING", "Select save point" )
        return
...
txt_dest_path = Entry(path_frame)   # Entry 가 아닌 text 였다면 "1.0"
...
# 간격 옵션 콤보
opt_space = ["Nothing", "Narrowly", "normal", "widely"]
